...
let localPreds = {};
let scored     = [];
let modalOnPick = null, modalSelected = null;
...
onValue(ref(db, "predictions"), snap => {
  ...
  // If current user is on predict screen but already submitted → redirect to dash
  if (me && allPreds[me.id] && document.getElementById("s-predict").classList.contains("active")) {
    showDash();
  }
});
...
function recomputeAndRender() {
  ...
  if (document.getElementById("s-dash").classList.contains("active")) {
    renderLeaderboard(); renderMyPicks(); renderAllPicks();
  }
}
...
function buildNameGrid() {
  ...
  updateNameGridBadges();
}

function updateNameGridBadges() {
  FAMILY.forEach(p => {
    const btn = document.getElementById(`namebtn-${p.id}`);
    if (!btn) return;
    const hasPred = !!allPreds[p.id];
    btn.classList.toggle("name-btn-done", hasPred);
    // Update inner content to show tick
    btn.innerHTML = `<span class="nb-emoji">${p.emoji}</span>${p.name}${hasPred ? '<span class="nb-done">✅</span>' : ''}`;
  });
}
...
function updateHeroBadge() {
  ...
  if (!el) return;
  if (gs.tossWinner) {
    el.textContent = `🪙 ${gs.tossWinner} won toss · ${gs.battingFirst || "?"} batting`;
...
function tryAdminLogin() {
  if (document.getElementById("admin-pw").value === "1234") {
    me = { id:"admin", name:"Admin", emoji:"⚙️" };
    show("s-admin");
    syncAdminFields();
...
function getMarginUnit() {
  ...
  const batting = gs.battingFirst;
  if (!winner) return null;
  if (!batting) return localPreds["q_margin_type"] || "runs";
...
function buildPredictForm() {
  ...
  QUESTIONS.forEach((q, idx) => {
    if (q.section && q.section !== lastSection) {
      ...
      sl.className = "section-label"; sl.textContent = q.section;
      wrap.appendChild(sl);
    }
    const card = document.createElement("div");
    card.className = "q-card"; card.id = `qcard-${q.id}`;
    card.innerHTML = `
      <div class="q-head">
...
function renderQBody(q) {
  ...
  if (q.type === "team") {
    body.innerHTML = `
      <div class="team-cards">
        <div class="team-card rcb-card ${val==="RCB"?"selected":""}" data-qid="${q.id}" data-val="RCB">
          <div class="tc-flag">🔴</div><div class="tc-short rcb-color">RCB</div>
        ...
        <div class="team-card gt-card ${val==="GT"?"selected":""}" data-qid="${q.id}" data-val="GT">
          <div class="tc-flag">🔵</div><div class="tc-short gt-color">GT</div>
    ...
    body.querySelectorAll(".team-card").forEach(tc => {
      tc.addEventListener("click", () => {
        ...
        renderQBody(q); markAnswered(q.id); updateProgress();
        if (q.id === "q_winner") {
  ...
  // ── CHOICE (bat/bowl) ──
    ...
    body.innerHTML = `
      <div class="team-cards">
        ${q.options.map((opt, i) => `
          <div class="choice-card ${val===opt?"selected":""}" data-qid="${q.id}" data-val="${opt}">
            <div class="tc-flag">${q.optionEmojis[i]}</div>
            <div class="tc-short" style="color:var(--text)">${opt}</div>
          </div>`).join("")}
      </div>`;
    body.querySelectorAll(".choice-card").forEach(cc => {
      cc.addEventListener("click", () => {
        localPreds[q.id] = cc.dataset.val;
        renderQBody(q); markAnswered(q.id); updateProgress();
      });
    });
  ...
  // ── MARGIN ──
    ...
    if (!winner) {
      body.innerHTML = `<div class="margin-no-winner">⬆️ Pick your Match Winner first to unlock this</div>`;
      if (hintEl) hintEl.textContent = "";
      return;
    }
    const unitLabel = unit === "wickets" ? "wickets" : "runs";
    const maxVal    = unit === "wickets" ? 10 : 150;
    if (hintEl) hintEl.textContent = `${winner} wins by … ${unitLabel}`;
    ...
    body.innerHTML = `
      ${showToggle ? `
        ...
        <div class="margin-unit-badge ${unit==="runs"?"runs-badge":"wkts-badge"}">
          ${unit==="runs" ? "🏏 Wins by RUNS" : "🎯 Wins by WICKETS"}
        </div>`}
      ${buildNumStepper(q.id, numVal, 1, maxVal, unitLabel)}`;
    if (showToggle) {
      body.querySelectorAll(".margin-type-btn").forEach(b => {
        b.addEventListener("click", () => { localPreds["q_margin_type"] = b.dataset.mt; renderQBody(q); });
      });
    }
    attachStepperListeners(body, q.id, 1, maxVal);
...
function attachStepperListeners(body, id, min, max, onChangeCb) {
  ...
  body.querySelectorAll(`[data-sid="${id}"]`).forEach(btn => {
    const fire = () => {
      ...
      let v = Math.max(min, Math.min(max, (parseInt(inp.value)||0) + parseInt(btn.dataset.dir)));
      inp.value = v; localPreds[id] = v;
  ...
  if (inp) {
    inp.addEventListener("input", () => {
      let v = parseInt(inp.value); if (isNaN(v)) return;
      v = Math.max(min, Math.min(max, v)); inp.value = v; localPreds[id] = v;
      if (onChangeCb) onChangeCb(); else { markAnswered(id); updateProgress(); }
    });
...
function refreshPredictForm() {
  QUESTIONS.forEach(q => { renderQBody(q); markAnswered(q.id, true); });
  updateProgress();
}
...
function markAnswered(qid, silent) {
  ...
  card.classList.toggle("answered", val !== undefined && val !== null && val !== "");
  if (!silent) card.classList.remove("error-highlight");
}

function updateProgress() {
  let done = 0;
  QUESTIONS.forEach(q => { if (localPreds[q.id] !== undefined && localPreds[q.id] !== null && localPreds[q.id] !== "") done++; });
...
function updatePredictLockUI() {
  ...
  document.querySelectorAll("#predict-content button, #predict-content input").forEach(el => el.disabled = locked);
}
...
function renderMiniScoreBar() {
  ...
  const gt=score?.gt||{}, rcb=score?.rcb||{};
  const fmt = t => (t.runs!==undefined&&t.runs!==null) ? `${t.runs}/${t.wickets??0} (${t.overs??0}ov)` : "—";
  const sMap={pre:"pre",live:"live",completed:"done"}, sLbl={pre:"Pre-Match",live:"● LIVE",completed:"Done"};
  bar.innerHTML = `
    <span class="msb-team rcb-color">RCB</span><span class="msb-score msb-rcb">${fmt(rcb)}</span>
    <span class="msb-sep">·</span>
    <span class="msb-team gt-color">GT</span><span class="msb-score msb-gt">${fmt(gt)}</span>
    ${battingFirst?`<span class="msb-sep">·</span><span style="font-size:.72rem;color:var(--t3)">${battingFirst} bat 1st</span>`:""}
    <span class="msb-status ${sMap[matchStatus]||"pre"}">${sLbl[matchStatus]||"Pre-Match"}</span>`;
}
...
document.getElementById("btn-submit").addEventListener("click", async () => {
  ...
  if (missing.length) {
    ...
    document.getElementById(`qcard-${missing[0].id}`)?.scrollIntoView({behavior:"smooth",block:"center"});
    return;
  }
  ...
  } catch(e) { showToast("❌ Save failed, try again"); console.error(e); }
});
...
function showDash() {
  ...
  renderLiveCard(); recomputeAndRender();
}
...
function renderLiveCard() {
  ...
  statusEl.className="lc-status "+cls; statusEl.textContent=txt;
  const info=document.getElementById("lc-innings-info");
  const lines=[];
  if (battingFirst) lines.push(`${battingFirst} batting 1st`);
  if (actuals?.q_winner) lines.push(`Winner: ${actuals.q_winner}${actuals.q_margin?` by ${actuals.q_margin} ${actuals.q_margin_type||"runs"}`:"" }`);
...
function renderLeaderboard() {
  ...
  const rows = FAMILY.map(p => ({
    ...p, ...(scoreMap[p.id]||{total:0,rank:FAMILY.length,breakdown:{}}), hasPred:!!allPreds[p.id],
  }));
  rows.sort((a,b)=>b.total-a.total);
  let html=`<div class="lb-wrap">`;
  rows.forEach((r,i) => {
    const isMe=me&&r.id===me.id, prize=prizes[r.id]??0;
...
function renderMyPicks() {
  ...
  if (!me || me.id==="admin") { panel.innerHTML=`<div class="results-note">Select a player to see their picks</div>`; return; }
  const preds=allPreds[me.id]||{};
  const myScore=scored.find(r=>r.playerId===me.id);
  const hasActuals=gs.actuals&&Object.keys(gs.actuals).some(k=>!k.includes("_type"));

  if (!Object.keys(preds).length) {
    panel.innerHTML=`<div class="results-note">⏳ You haven't submitted predictions yet</div>`; return;
  }
  ...
  QUESTIONS.forEach((q,idx) => {
    ...
    if (hasActuals&&gs.actuals?.[q.id]) {
      const cls={correct:"pts-correct",wrong:"pts-wrong",exact:"pts-exact",closest:"pts-closest",off:"pts-off",no_pred:"pts-wrong"}[label]||"pts-pending";
...
function formatPredDisplay(q, preds) {
  ...
  if (q.type==="margin") return `${v} ${preds?.[`${q.id}_type`]||"runs"}`;
  return String(v);
}
...
function renderAllPicks() {
  ...
  const scoreMap={};
  scored.forEach(r=>scoreMap[r.playerId]=r);
  const sortedFamily=[...FAMILY].sort((a,b)=>(scoreMap[b.id]?.total||0)-(scoreMap[a.id]?.total||0));
  let html="";

  if (!hasActuals) {
    html+=`<div class="results-note">⏳ Results appear here after admin fills final answers</div>`;
    QUESTIONS.forEach((q,idx) => {
      ...
      FAMILY.forEach(p => {
        ...
        const display=pred!==undefined&&pred!==null&&pred!==""?formatPredDisplay(q,allPreds[p.id]):null;
    ...
    QUESTIONS.forEach((q,idx) => {
      ...
      const picks=sortedFamily.map(p => {
        ...
        const pts=(br?.pts||0)+(br?.bonus||0);
        const display=(pred!==undefined&&pred!==null&&pred!=="")?formatPredDisplay(q,allPreds[p.id]):null;
        return {p,display,label,pts,diff:br?.diff};
      });
      ...
      picks.forEach(({p,display,label,pts,diff}) => {
        const isOk=["correct","exact","closest"].includes(label);
        const icon=label==="exact"?"🎯":label==="closest"?"📍":isOk?"✅":label==="empty"||label==="no_pred"?"—":"❌";
        const rowCls=label==="exact"?"res-row exact":label==="closest"?"res-row closest":isOk?"res-row correct":"res-row wrong";
        const ptsTxt=pts>0?`+${pts}`:label==="off"&&diff?`off ${diff}`:label==="empty"||label==="no_pred"?"—":"0";
        ...
        html+=`<div class="${rowCls}">
          ...
          <span class="res-pred">${display||"<span style='color:var(--t3)'>—</span>"}</span>
    ...
    html+=`<div class="res-summary-head">🏆 Final Standings</div><div class="res-summary">`;
    const {prizes}=calcPrizes(scored);
    const rankEmoji=["🥇","🥈","🥉","4️⃣","5️⃣","6️⃣","7️⃣","8️⃣","9️⃣","🔟","1️⃣1️⃣"];
    sortedFamily.forEach((p,i)=>{
      const s=scoreMap[p.id], pts=s?.total||0, prize=prizes[p.id]??0;
      ...
      html+=`<div class="res-summary-row ${isMe?"me":""}">
        ...
        <span class="rs-money">${prize>0?`₹${prize}`:"—"}</span>
      </div>`;
    });
    html+=`</div>`;
  }
...
function formatActualDisplay(q,actuals) {
  ...
  if (!v&&v!==0) return "";
  if (q.type==="margin") return `${v} ${actuals?.[`${q.id}_type`]||"runs"}`;
  return String(v);
}
...
// ─── PLAYER MODAL ─────────────────────────────────────────────────────────────
function openModal(q, forceFilter) {
  const filter=forceFilter||q.filter||"all";
  modalSelected=localPreds[q.id]?findPlayer(localPreds[q.id]):null;
  ...
  modalOnPick=(player)=>{
    ...
    markAnswered(q.id); updateProgress(); renderQBody(q);
  };
}
...
function renderTeamInModal(containerId,squad,team) {
  ...
  const gn={bat:"🏏 Batters / WK",ar:"⚡ All-Rounders",bowl:"🎯 Bowlers"};
  let html="";
  for (const role of ["bat","ar","bowl"]) {
    const players=squad.filter(p=>p.role===role);
    if (!players.length) continue;
    html+=`<div class="role-group"><div class="role-group-label">${gn[role]}</div><div class="player-chips">`;
    players.forEach(p=>{
      const isSel=modalSelected?.name===p.name;
      html+=`<button class="p-chip ${isSel?`sel-${team}`:""}" data-name="${p.name}" data-team="${team.toUpperCase()}">${p.name}<span class="p-chip-role">${p.roleLabel}</span></button>`;
    });
    html+=`</div></div>`;
  }
  el.innerHTML=html;
...
document.getElementById("modal-confirm").addEventListener("click",()=>{
  ...
  modalOnPick&&modalOnPick(modalSelected); closeModal();
});
...
function syncAdminFields() {
  ...
  sv("a-toss",gs.tossWinner); sv("a-bat",gs.battingFirst); sv("a-status",gs.matchStatus);
  sv("a-rcb-r",gs.score?.rcb?.runs); sv("a-rcb-w",gs.score?.rcb?.wickets);
  sv("a-rcb-o",gs.score?.rcb?.overs); sv("a-rcb-status",gs.score?.rcb?.status);
  sv("a-gt-r",gs.score?.gt?.runs);  sv("a-gt-w",gs.score?.gt?.wickets);
  sv("a-gt-o",gs.score?.gt?.overs);  sv("a-gt-status",gs.score?.gt?.status);
  if (gs.actuals) {
    QUESTIONS.forEach(q=>{
      if (q.type==="margin"){sv(`ar-${q.id}`,gs.actuals[q.id]);sv(`ar-${q.id}_type`,gs.actuals[`${q.id}_type`]);}
      else sv(`ar-${q.id}`,gs.actuals[q.id]);
    });
  }
}
...
function buildAdminResultsForm() {
  ...
  QUESTIONS.forEach(q=>{
    ...
    const lbl=document.createElement("div"); lbl.className="ar-label"; lbl.textContent=q.label;
    wrap.appendChild(lbl);

    if (q.type==="team") {
      wrap.appendChild(mkSel(`ar-${q.id}`,[["","— Select —"],["RCB","RCB"],["GT","GT"]]));
    } else if (q.type==="choice") {
      const s=mkSel(`ar-${q.id}`,[["","— Select —"],...q.options.map(o=>[o,o])]);
      wrap.appendChild(s);
    } else if (q.type==="margin") {
      const row=document.createElement("div"); row.style.cssText="display:flex;gap:8px;";
      const ni=mkInput(`ar-${q.id}`,"number","e.g. 24"); ni.min="1"; ni.max="200"; ni.style.flex="1";
      const ts=mkSel(`ar-${q.id}_type`,[["runs","Runs"],["wickets","Wickets"]]); ts.style.flex="1";
      if (gs.battingFirst&&gs.actuals?.q_winner)
        ts.value=gs.actuals.q_winner===gs.battingFirst?"runs":"wickets";
      row.appendChild(ni); row.appendChild(ts); wrap.appendChild(row);
    } else if (q.type==="player") {
      const inp=mkInput(`ar-${q.id}`,"text","Player name");
      inp.setAttribute("list",`dl-${q.id}`);
      const dl=document.createElement("datalist"); dl.id=`dl-${q.id}`;
      let pool=[...RCB_SQUAD,...GT_SQUAD];
      if (q.filter==="batAR")  pool=[...rcbBatAR(),...gtBatAR()];
      if (q.filter==="bowlAR") pool=[...rcbBowlAR(),...gtBowlAR()];
      pool.forEach(p=>{const o=document.createElement("option");o.value=p.name;dl.appendChild(o);});
      wrap.appendChild(dl); wrap.appendChild(inp);
    } else if (q.type==="number") {
      const inp=mkInput(`ar-${q.id}`,"number","0"); inp.min=String(q.min); inp.max=String(q.max);
...
document.getElementById("btn-save-score").addEventListener("click",async()=>{
  try{
    await update(ref(db,"gameState/score"),{
      rcb:{runs:parseInt(document.getElementById("a-rcb-r").value)||0,wickets:parseInt(document.getElementById("a-rcb-w").value)||0,overs:parseFloat(document.getElementById("a-rcb-o").value)||0,status:document.getElementById("a-rcb-status").value},
      gt:{runs:parseInt(document.getElementById("a-gt-r").value)||0,wickets:parseInt(document.getElementById("a-gt-w").value)||0,overs:parseFloat(document.getElementById("a-gt-o").value)||0,status:document.getElementById("a-gt-status").value},
    });
...
document.getElementById("btn-save-results").addEventListener("click",async()=>{
  ...
  try{
    ...
    showToast("🏁 Results saved — scores live!");
  }catch(e){showToast("❌ Error");console.error(e);}
});
...
function findPlayer(name){
  return [...RCB_SQUAD,...GT_SQUAD].find(p=>p.name===name)||(name?{name,team:"?",roleLabel:"?"}:null);
}
let toastTmr;
function showToast(msg){
  const t=document.getElementById("toast");
  t.textContent=msg;t.classList.remove("hidden");
  clearTimeout(toastTmr);toastTmr=setTimeout(()=>t.classList.add("hidden"),2800);
}